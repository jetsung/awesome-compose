...
| 状态 | 项目名称 | 描述 |
| --- | --- | --- |
|   | [acme](./acme/) | 自动化证书管理环境，用于自动获取和续期 SSL/TLS 证书 |
|   | [adminer](./adminer/) | 开源数据库管理工具，允许用户通过Web界面管理数据库，如MySQL、SQLite等 |
|   | [aliyundrive](./aliyundrive/) | 阿里云盘 WebDAV 服务，将阿里云盘挂载为标准 WebDAV 文件系统 |
|   | [asciinema](./asciinema/) | 用于录制和分享终端会话的工具，将终端操作录制成可播放的动画 |
|   | [bark](./bark/) | 注重隐私、安全可控的自定义通知推送工具 |
|   | [bore](./bore/) | 简单易用的内网穿透工具，用于将本地服务暴露到公网 |
|   | [chanify](./chanify/) | 安全的消息推送服务，支持多平台通知推送 |
|   | [claude-code-router](./claude-code-router/) | 代码路由工具 |
|   | [croc](./croc/) | 跨平台文件传输工具，可在两台计算机之间安全地传输文件 |
|   | [drone](./drone/) | 基于容器技术的持续集成和持续部署(CI/CD)平台 |
|   | [dufs](./dufs/) | 基于 HTTP 的文件服务器，支持文件上传、下载和目录浏览 |
|   | [emqx](./emqx/) | 大规模分布式物联网接入平台，高性能、可扩展的 MQTT 消息服务器 |
|   | [etcd](./etcd/) | 高可用的分布式键值存储系统，常用于配置共享和服务发现 |
|   | [ferron](./ferron/) | 基于 Zola 的静态网站生成器 |
|   | [file-downloader](./file-downloader/) | 文件下载工具 |
|   | [filetas](./filetas/) | 文件管理和分享服务 |
|   | [frp](./frp/) | 专注内网穿透的高性能反向代理应用 |
|   | [gitea](./gitea/) | 轻量级的 DevOps 平台软件，提供 Git 托管、代码审查、团队协作等功能 |
|   | [gitlab-runner](./gitlab-runner/) | GitLab CI/CD 的执行器，用于运行 CI/CD 任务 |
...
|   | [komodo](./komodo/) | 现代化的代码编辑器 |
|   | [linkding](./linkding/) | 简单、快速的自托管书签服务 |
...
|   | [memos](./memos/) | 隐私优先、自托管的知识库 |
|   | [mindoc](./mindoc/) | 基于 Go 的简约文档管理系统 |
|   | [minio](./minio/) | 高性能的对象存储服务，兼容 Amazon S3 API |
...
|   | [openssh-server](./openssh-server/) | SSH 服务器，用于远程安全连接 |
...
|   | [static-web-server](./static-web-server/) | 高性能的静态 Web 服务器 |
|   | [swagger](./swagger/) | 用于设计、构建和文档化 RESTful API 的工具 |
|   | [syncthing](./syncthing/) | 连续的文件同步程序，用于在不同设备间同步文件 |
|   | [transfer](./transfer/) | 文件传输服务 |
|   | [valkey](./valkey/) | 开源的、内存中的数据存储，用于高速数据处理 |
|   | [vaultwarden](./vaultwarden/) | Bitwarden 密码管理器的非官方服务器实现 |
|   | [vaultwarden-backup](./vaultwarden-backup/) | 为 Vaultwarden 提供备份功能的服务 |
|   | [webdav](./webdav/) | 基于 Web 的分布式文件管理系统 |
|   | [wechat-devtools](./wechat-devtools/) | 微信开发者工具 |
|   | [wingetty](./wingetty/) | Windows 终端工具 |
|   | [woodpecker](./woodpecker/) | 简单、强大、轻量级的 CI/CD 引擎 |
|   | [wordpress](./wordpress/) | 功能强大的开源内容管理系统 |
